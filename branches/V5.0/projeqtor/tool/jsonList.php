...
if ($type == 'list' and array_key_exists ( 'dataType', $_REQUEST ) and $_REQUEST ['dataType'] == 'idResource' and array_key_exists ( 'critField', $_REQUEST ) and array_key_exists ( 'critValue', $_REQUEST ) and $_REQUEST ['critField'] == 'idProject') {
...
if ($type == 'empty') {
  ...
  if ($dataType == 'idProject' and securityGetAccessRight ( 'menuProject', 'read' ) != 'ALL') {
    ...
    if (trim ( $_REQUEST ['critValue'] )) {
      ...
      $versProjList = $versProj->getSqlElementsFromCriteria ( array (
          'idProject' => $_REQUEST ['critValue'] 
      ) );
    ...
    $crit = array (
        $_REQUEST ['critField'] => $_REQUEST ['critValue'] 
    );
  ...
  if (! isset ( $_REQUEST ['selected'] )) {
    ...
    foreach ( $list as $term ) {
      ...
      $dep = new Dependency ();
      $crit = array (
          "successorRefType" => "Term",
          "successorRefId" => $term->id 
      );
...
function listFieldsForFilter($obj, $nbRows, $included = false) {
  ...
  foreach ( $obj as $col => $val ) {
    if (substr ( $col, 0, 1 ) != "_" and substr ( $col, 0, 1 ) != ucfirst ( substr ( $col, 0, 1 ) ) and ! $obj->isAttributeSetToField ( $col, 'hidden' ) and ! $obj->isAttributeSetToField ( $col, 'calculated' ) and (! $included or ($col != 'id' and $col != 'refType' and $col != 'refId' and $col != 'idle'))) {
      if ($nbRows > 0)
        echo ', ';
