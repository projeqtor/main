...
if ($quickSearch) {
  ...
  $queryWhere .= " or exists ( select 'x' from $noteTable " . " where $noteTable.refType=" . Sql::str ( $objectClass ) . " and $noteTable.refId=$table.id " . " and $noteTable.note " . ((Sql::isMysql ()) ? 'LIKE' : 'ILIKE') . " '%" . $quickSearch . "%' ) ";
  $queryWhere .= " )";
}
  ...
  foreach ( $arrayFilter as $crit ) {
    if ($crit ['sql'] ['operator'] == 'SORT') {
      ...
      if (count ( $split ) > 1) {
        ...
        $queryOrderBy .= ($queryOrderBy == '') ? '' : ', ';
        $queryOrderBy .= " " . $externalTableAlias . '.' . $split [1] . " " . $crit ['sql'] ['value'];
        $doneSort = true;
      }
  ...
  foreach ( $array as $val ) {
    ...
    if ($fld) {
      ...
      if (strlen ( $fld ) > 9 and substr ( $fld, 0, 9 ) == "colorName") {
        ...
        if (property_exists ( $externalObj, 'wbsSortable' ) and strpos ( $queryOrderBy, $externalTableAlias . "." . $externalObj->getDatabaseColumnName ( 'wbsSortable' ) ) === false) {
          $queryOrderBy .= ($queryOrderBy == '') ? '' : ', ';
...
foreach ( $arrayFilter as $crit ) {
  if ($crit ['sql'] ['operator'] != 'SORT') { // Sorting already applied above
    ...
    if ($crit ['sql'] ['operator'] == 'IN' and $crit ['sql'] ['attribute'] == 'idProduct') {
      $critSqlValue = str_replace ( array (
          ' ',
          '(',
          ')' 
      ), '', $critSqlValue );
    ...
    if (count ( $split ) > 1) {
      ...
      $queryFrom .= ' left join ' . $externalTable . ' as ' . $externalTableAlias . ' on ( ' . $externalTableAlias . ".refType='" . get_class ( $obj ) . "' and " . $externalTableAlias . '.refId = ' . $table . '.id )';
      $queryWhere .= ($queryWhere == '') ? '' : ' and ';
      $queryWhere .= $externalTableAlias . "." . $split [1] . ' ' . $crit ['sql'] ['operator'] . ' ' . $critSqlValue;
    } else {
      $queryWhere .= ($queryWhere == '') ? '' : ' and ';
      ...
      if (strlen ( $crit ['sql'] ['attribute'] ) >= 9 and substr ( $crit ['sql'] ['attribute'], 0, 2 ) == 'id' and substr ( $crit ['sql'] ['attribute'], - 7 ) == 'Version' and $crit ['sql'] ['operator'] == 'IN') {
        $scope = substr ( $crit ['sql'] ['attribute'], 2 );
        $vers = new OtherVersion ();
        $queryWhere .= " or exists (select 'x' from " . $vers->getDatabaseTableName () . " VERS " . " where VERS.refType=" . Sql::str ( $objectClass ) . " and VERS.refId=" . $table . ".id and scope=" . Sql::str ( $scope ) . " and VERS.idVersion IN " . $critSqlValue . ")";
      }
...
$query = 'select ' . $querySelect . ' from ' . $queryFrom . ' where ' . $queryWhere . ' order by' . $queryOrderBy;
// --- Execute query
      ...
      foreach ( $line as $id => $val ) {
        ...
        if (substr ( $id, 0, 2 ) == 'id' and strlen ( $id ) > 2) {
          ...
          if (ucfirst ( $class ) == $class) {
            $foreign = true;
            if ($class == "TargetVersion" or $class == "OriginalVersion")
              $class = 'Version';
    ...
    if (Sql::$lastQueryNbRows > 0) {
      ...
      while ( $line = Sql::fetchLine ( $result ) ) {
        ...
        foreach ( $line as $id => $val ) {
          $numField += 1;
          $disp = "";
          if (! isset ( $arrayWidth [$numField] ) or $arrayWidth [$numField] == '')
            continue;
