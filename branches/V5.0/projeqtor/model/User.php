...
class User extends SqlElement {
  ...
  public function getAccessRights($class, $right = null) {
    ...
    if (! $this->_allAccessRights)
      $this->_allAccessRights = array ();
  ...
  public static function resetAllVisibleProjects($idProject = null, $idUser = null) {
    ...
    if ($idUser) {
      if ($idUser == $user->id) {
        ...
        $audit = new Audit ();
        $auditList = $audit->getSqlElementsFromCriteria ( array (
            "idUser" => $idUser,
            'idle' => '0' 
        ) );
      ...
      $aff = new Affectation ();
      $affList = $aff->getSqlElementsFromCriteria ( array (
          'idProject' => $idProject 
      ) );
      foreach ( $affList as $aff ) {
  ...
  public function control() {
    ...
    if ($this->isResource and (! $this->resourceName or $this->resourceName == "")) {
      $result .= '<br/>' . i18n ( 'messageMandatory', array (
          i18n ( 'colresourceName' ) 
      ) );
    }
    $crit = array (
        "name" => $this->name 
  ...
  public function authenticate($paramlogin, $parampassword) {
    ...
    if ($this->isLdap == 0) {
      ...
      $filter_r = html_entity_decode ( str_replace ( array (
          '%USERNAME%',
          '%username%' 
      ), array (
          $this->name,
          $this->name 
      ), $paramLdap_user_filter ), ENT_COMPAT, 'UTF-8' );
      ...
      if (! $this->id and $this->isLdap) {
        if (! count ( $first_user ) == 0) {
          ...
          if ($sendAlert != 'NO') {
            ...
            $message = i18n ( "newUserMessage", array (
                $paramlogin 
            ) );
  ...
  public function stopAllWork() {
    $we = new WorkElement ();
    $weList = $we->getSqlElementsFromCriteria ( array (
        'idUser' => $this->id,
        'ongoing' => '1' 
    ) );
  ...
  public function getPhotoThumb($size) {
    ...
    if ($image->id and $image->isThumbable ()) {
      $result .= '<img src="' . getImageThumb ( $image->getFullPathFileName (), $size ) . '" ' . ' title="' . $image->fileName . '" style="cursor:pointer"' . ' onClick="showImage(\'Attachment\',\'' . $image->id . '\',\'' . $image->fileName . '\');" />';
    } else {
  ...
  public function getListOfPlannableProjects() {
    ...
    $canPlan = false;
    $right = SqlElement::getSingleSqlElementFromCriteria ( 'habilitationOther', array (
        'idProfile' => $defProfile,
        'scope' => 'planning' 
    ) );
    if ($right) {
