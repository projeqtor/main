    ...
    substr($col, 0, 6) != '_void_' and substr($col, 0, 7) != '_label_' and substr($col, 0, 8) != '_button_') { // field not to be displayed
      ...
      if (is_object($val)) {
        ...
        if ($hide) { // hidden field
          ...
          if ($outMode == 'pdf') {
            echo htmlRemoveDocumentTags(htmlEncode($val,'formatted'));
          } else {
            echo htmlEncode($val,'formatted');
          }
        } else if ($col == 'id') { // id
        ...
        if ($col == 'idResource' and $next and $showExtraButton) {
          ...
          echo ' style="text-align:left;float:right;margin-right:10px; width:' . ($fieldWidth - 5) . 'px"';
          echo ' onClick="dijit.byId(\'' . $fieldId . '\').set(\'value\',' . $user->id . ');setTimeout(\'saveObject()\',100);"';
          echo '>';
          echo '<img src="css/images/iconMoveTo.png" style="position:relative;left:5px;top:2px;"/>';
