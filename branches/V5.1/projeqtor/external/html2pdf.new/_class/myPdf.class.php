...
if (!defined('HTML2PDF_USED_TCPDF_VERSION')) {
    if (defined('PDF_PRODUCER')) {
        define('HTML2PDF_USED_TCPDF_VERSION', preg_replace('/^.*? (\d[0-9.]*).*$/', '\1', PDF_PRODUCER));
    } else {
        require_once(K_PATH_MAIN . 'include/tcpdf_static.php');
        define('HTML2PDF_USED_TCPDF_VERSION', TCPDF_STATIC::getTCPDFVersion());
    }
}
...
class HTML2PDF_myPdf extends TCPDF
{
        ...
        $fontName = 'helvetica')
    {
        ...
        for ($i=0;$i<$size;$i++) {
            ...
            //Insert a link.
            $link = $this->AddLink();
            $this->SetLink($link, $this->outlines[$i]['y'], $this->outlines[$i]['p']);

            // if we want to display the page nmber
            if ($displayPage) {
                // display the Bookmark Caption
                $this->Cell($strsize+2, $this->FontSize+2, $str, 0, 0, '', false, $link);
                ...
                //Page number
                $this->Cell($pageCellSize, $this->FontSize+2, 'p. '.$this->outlines[$i]['p'], 0, 1, 'R', false, $link);
            } else {
                // display the Bookmark Caption
                $this->Cell($strsize+2, $this->FontSize+2, $str, 0, 1, '', false, $link);
            }
        }
    }
