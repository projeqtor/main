...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    function artxPost($data)
    {
    	...
    	artxFragmentBegin(str_replace('class="td-box td-post">', 'class="td-box td-post' . $classes . '">', "<div class=\"td-box td-post\">\r\n    <div class=\"td-box-body td-post-body\">\r\n<div class=\"td-post-inner\">\r\n"));
    	artxFragmentBegin("<h2 class=\"td-postheader\">");
    	artxFragmentBegin("");
...
<div class="td-box td-block">
        ...
        <?php echo str_replace('class="td-box td-block">', 'class="td-box td-block' . $classes . '">', ob_get_clean()); ?>
        <?php if ($hasCaption): ?>
...
<?php
    ...
    function artxModules(&$document, $position, $style = null)
    {
         ...
         *  Select inputs with class != 'td-button'
         *   artxFindTags($html, array('input' => array('class' => '!td-button')))
         *  Select inputs with non-empty class
         *   artxFindTags($html, array('input' => array('class' => '!')))
         *  Select inputs with class != 'td-button' and non-empty class:
         *   artxFindTags($html, array('input' => array('class' => array('!td-button', '!'))))
         *  Select inputs with class = button but != 'td-button'
         *   artxFindTags($html, array('input' => array('class' => array('button', '!td-button'))))
         *
        ...
        function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
        {
            ...
            foreach (artxFindTags($content, $filter) as $tag) {
                if (is_string($tag))
                    ...
                    $tag['attributes']['class']['value'][] = 'td-button';
                    $delimeter = '' == $tag['attributes']['class']['delimeter']
                        ? '"' : $tag['attributes']['class']['delimeter'];
                    $tag['attributes']['class']['delimeter'] = $delimeter;
                    $button = str_replace('"', $delimeter, '<span class="td-button-wrapper">'
                        . '<span class="td-button-l"> </span><span class="td-button-r"> </span>')
        ...
        {
            return '<span class="td-button-wrapper"><span class="td-button-l"> </span><span class="td-button-r"> </span>'
                . '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
                . 'td-button" href="' . $data['link'] . '">' . $data['content'] . '</a></span>';
        }
...
function artxPostprocessBlockContent($content)
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!td-button')),
            'button' => array('class' => array('button', '!td-button'))));
        return $content;
    }
