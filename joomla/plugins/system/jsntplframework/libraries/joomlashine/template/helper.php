...
class JSNTplTemplateHelper
{
	...
	private function _prepare ($loadTemplateCSS, $loadTemplateJS)
	{
		...
		if ($params['colorSelector'] AND ! isset($params['sitetoolsColorsItems']))
		{
		...
		foreach (array('promoColumns', 'mainColumns', 'contentColumns', 'userColumns') AS $row)
		{
			...
			{
				// Re-fine ID
				$real_id = preg_replace('/^\d+:/', '', $id);

				// Detect the visibility of this column
				if ( ! in_array($real_id, array('content', 'component')) AND ! $this->_document->countModules($real_id))
				{
					$visible -= 1;
			...
			foreach ($columns AS $column)
			{
				...
				// Re-fine ID
				$real_id = preg_replace('/^\d+:/', '', $column['id']);
	...
	private function _prepareHead ()
	{
		...
		$this->_document->addScript($this->_document->rootUrl . '/plugins/system/jsntplframework/assets/3rd-party/jquery/jquery-1.8.2.js');
		$this->_document->addScript($this->_document->rootUrl . '/plugins/system/jsntplframework/assets/3rd-party/bootstrap/js/bootstrap.min.js');

		// Init Bootstrap dropdown menu
		$this->_document->addScriptDeclaration('jQuery(document).ready(function() { var dropDown = jQuery(".dropdown-toggle"); if (dropDown.length) dropDown.dropdown(); });');
