...
abstract class KunenaForum {
	...
	protected static function buildVersion() {
		if ('3.0.2' == '@' . 'kunenaversion' . '@') {
			...
			self::$version = strtoupper ( '3.0.2' );
		}
		self::$version_major = substr(self::$version, 0, 3);
		self::$version_date = ('2013-08-18' == '@' . 'kunenaversiondate' . '@') ? JFactory::getDate()->format('Y-m-d') : '2013-08-18';
		self::$version_name = ('Nocturne' == '@' . 'kunenaversionname' . '@') ? 'Git Repository' : 'Nocturne';
	}
}
