...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF
    {
        ...
        protected $_subobj           = null;        // object html2pdf prepared in order to accelerate the creation of sub html2pdf
        protected $_tables;                         // table to prepare the nested html tables
        ...
        /**
         * Clone to create a sub HTML2PDF from $this->_subobj
         *
         * @access public
         */
        public function __clone()
        {
            // TCPDF lacks a good deep cloning support, thus do it here lazily.
...
'.$content.'
        ...
        protected function _prepareSubObj()
        {
            ...
            // init
            $this->_subobj->_tables = &$this->_tables;
            $this->_subobj->setTestTdInOnePage($this->_testTdInOnepage);
            $this->_subobj->setTestIsImage($this->_testIsImage);
            $this->_subobj->setTestIsDeprecated($this->_testIsDeprecated);
            $this->_subobj->setDefaultFont($this->_defaultFont);
            $this->_subobj->parsingCss->css            = &$this->parsingCss->css;
            $this->_subobj->parsingCss->cssKeys        = &$this->parsingCss->cssKeys;

            // clone font from the original PDF
            $this->_subobj->pdf->cloneFontFrom($this->pdf);

            // remove the link to the parent
            $this->_subobj->parsingCss->setPdfParent($pdf);
        }
        ...
        protected function _createSubHTML(&$subHtml, $cellmargin=0)
        {
            ...
            $this->_subobj->pdf->getPage();

            // clone the sub oject
            $subHtml = clone $this->_subobj;
            $subHtml->_tables = &$this->_tables;
        ...
        public function _tag_open_NOBR ($param) {
          return $this->_tag_open_NOBREAK($param);
        }
        public function _tag_close_NOBR ($param) {
          return $this->_tag_close_NOBREAK($param);
        }
        /**
         * tag : DIV
         * mode : OPEN
        ...
        protected function _tag_open_HR($param)
        {
            ...
            // Restore font size in producer.
            $this->pdf->SetFontSize(720 * $fontSize / 254);
        ...
        protected function _tag_open_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                $this->_tables[$param['num']]['thead']['tr'][0] = $this->_tables[$param['num']]['tr_curr'];
                $this->_tables[$param['num']]['thead']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    ...
                    $this->_tables[$param['num']]['thead']['code'][] = $action;
                    if (strtolower($action['name'])=='thead_sub' && $action['close']) break;
                }
            } else {
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count($level);
                $this->_tables[$param['num']]['tr_curr']+= count($this->_tables[$param['num']]['thead']['tr']);
            }
        ...
        protected function _tag_close_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                $min = $this->_tables[$param['num']]['thead']['tr'][0];
                $max = $this->_tables[$param['num']]['tr_curr']-1;
                $this->_tables[$param['num']]['thead']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TFOOT($param)
        {
            ...
            if ($this->_subPart) {
                $this->_tables[$param['num']]['tfoot']['tr'][0] = $this->_tables[$param['num']]['tr_curr'];
                $this->_tables[$param['num']]['tfoot']['code'] = array();
                for ($pos=$this->_tempPos; $pos<count($this->parsingHtml->code); $pos++) {
                    $action = $this->parsingHtml->code[$pos];
                    if (strtolower($action['name'])=='tfoot') $action['name'] = 'tfoot_sub';
                    $this->_tables[$param['num']]['tfoot']['code'][] = $action;
                    if (strtolower($action['name'])=='tfoot_sub' && $action['close']) break;
                }
            } else {
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count($level);
                $this->_tables[$param['num']]['tr_curr']+= count($this->_tables[$param['num']]['tfoot']['tr']);
            }
        ...
        protected function _tag_close_TFOOT($param)
        {
            ...
            if ($this->_subPart) {
                $min = $this->_tables[$param['num']]['tfoot']['tr'][0];
                $max = $this->_tables[$param['num']]['tr_curr']-1;
                $this->_tables[$param['num']]['tfoot']['tr'] = range($min, $max);
            }
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_tables[$param['num']] = array();
                $this->_tables[$param['num']]['border']          = isset($param['border']) ? $this->parsingCss->readBorder($param['border']) : null;
                $this->_tables[$param['num']]['cellpadding']     = $this->parsingCss->ConvertToMM(isset($param['cellpadding']) ? $param['cellpadding'] : '1px');
                $this->_tables[$param['num']]['cellspacing']     = $this->parsingCss->ConvertToMM(isset($param['cellspacing']) ? $param['cellspacing'] : '2px');
                $this->_tables[$param['num']]['cases']           = array();          // properties of each TR/TD
                $this->_tables[$param['num']]['corr']            = array();          // link between TR/TD and colspan/rowspan
                $this->_tables[$param['num']]['corr_x']          = 0;                // position in 'cases'
                $this->_tables[$param['num']]['corr_y']          = 0;                // position in 'cases'
                $this->_tables[$param['num']]['td_curr']         = 0;                // current column
                $this->_tables[$param['num']]['tr_curr']         = 0;                // current row
                $this->_tables[$param['num']]['curr_x']          = $this->pdf->getX();
                $this->_tables[$param['num']]['curr_y']          = $this->pdf->getY();
                $this->_tables[$param['num']]['width']           = 0;                // global width
                $this->_tables[$param['num']]['height']          = 0;                // global height
                $this->_tables[$param['num']]['align']           = $alignObject;
                $this->_tables[$param['num']]['marge']           = array();
                $this->_tables[$param['num']]['marge']['t']      = $this->parsingCss->value['padding']['t']+$this->parsingCss->value['border']['t']['width']+$this->_tables[$param['num']]['cellspacing']*0.5;
                $this->_tables[$param['num']]['marge']['r']      = $this->parsingCss->value['padding']['r']+$this->parsingCss->value['border']['r']['width']+$this->_tables[$param['num']]['cellspacing']*0.5;
                $this->_tables[$param['num']]['marge']['b']      = $this->parsingCss->value['padding']['b']+$this->parsingCss->value['border']['b']['width']+$this->_tables[$param['num']]['cellspacing']*0.5;
                $this->_tables[$param['num']]['marge']['l']      = $this->parsingCss->value['padding']['l']+$this->parsingCss->value['border']['l']['width']+$this->_tables[$param['num']]['cellspacing']*0.5;
                $this->_tables[$param['num']]['page']            = 0;                // number of pages
                $this->_tables[$param['num']]['new_page']        = true;             // flag : new page for the current TR
                $this->_tables[$param['num']]['style_value']     = null;             // CSS style of the table
                $this->_tables[$param['num']]['thead']           = array();          // properties on the thead
                $this->_tables[$param['num']]['tfoot']           = array();          // properties on the tfoot
                $this->_tables[$param['num']]['thead']['tr']     = array();          // list of the TRs in the thead
                $this->_tables[$param['num']]['tfoot']['tr']     = array();          // list of the TRs in the tfoot
                $this->_tables[$param['num']]['thead']['height']    = 0;             // thead height
                $this->_tables[$param['num']]['tfoot']['height']    = 0;             // tfoot height
                $this->_tables[$param['num']]['thead']['code'] = array();            // HTML content of the thead
                $this->_tables[$param['num']]['tfoot']['code'] = array();            // HTML content of the tfoot
                $this->_tables[$param['num']]['cols']        = array();              // properties of the COLs

                $this->_saveMargin($this->pdf->getlMargin(), $this->pdf->gettMargin(), $this->pdf->getrMargin());

                $this->parsingCss->value['width']-= $this->_tables[$param['num']]['marge']['l'] + $this->_tables[$param['num']]['marge']['r'];
            } else {
                // we start from the first page and the first page of the table
                $this->_tables[$param['num']]['page'] = 0;
                $this->_tables[$param['num']]['td_curr']    = 0;
                $this->_tables[$param['num']]['tr_curr']    = 0;
                $this->_tables[$param['num']]['td_x']        = $this->_tables[$param['num']]['marge']['l']+$this->_tables[$param['num']]['curr_x'];
                $this->_tables[$param['num']]['td_y']        = $this->_tables[$param['num']]['marge']['t']+$this->_tables[$param['num']]['curr_y'];

                // draw the borders/background of the first page/part of the table
                $this->_drawRectangle(
                    $this->_tables[$param['num']]['curr_x'],
                    $this->_tables[$param['num']]['curr_y'],
                    $this->_tables[$param['num']]['width'],
                    isset($this->_tables[$param['num']]['height'][0]) ? $this->_tables[$param['num']]['height'][0] : null,
                    $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    0,
                    $this->parsingCss->value['background']
                ...
                $this->_tables[$param['num']]['style_value'] = $this->parsingCss->value;
            }
        ...
        protected function _tag_close_TABLE($param)
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_calculateTableCellSize($this->_tables[$param['num']]['cases'], $this->_tables[$param['num']]['corr']);

                // calculate the height of the thead and the tfoot
                $lst = array('thead', 'tfoot');
                foreach ($lst as $mode) {
                    $this->_tables[$param['num']][$mode]['height'] = 0;
                    foreach ($this->_tables[$param['num']][$mode]['tr'] as $tr) {
                        // hauteur de la ligne tr
                        $h = 0;
                        for ($i=0; $i<count($this->_tables[$param['num']]['cases'][$tr]); $i++)
                            if ($this->_tables[$param['num']]['cases'][$tr][$i]['rowspan']==1)
                                $h = max($h, $this->_tables[$param['num']]['cases'][$tr][$i]['h']);
                        $this->_tables[$param['num']][$mode]['height']+= $h;
                    }
                }

                // calculate the width of the table
                $this->_tables[$param['num']]['width'] = $this->_tables[$param['num']]['marge']['l'] + $this->_tables[$param['num']]['marge']['r'];
                if (isset($this->_tables[$param['num']]['cases'][0])) {
                    foreach ($this->_tables[$param['num']]['cases'][0] as $case) {
                        $this->_tables[$param['num']]['width']+= $case['w'];
                    }
                }
                ...
                $old = $this->parsingCss->getOldValues();
                $parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();
                $x = $this->_tables[$param['num']]['curr_x'];
                $w = $this->_tables[$param['num']]['width'];
                if ($parentWidth>$w) {
                    if ($this->_tables[$param['num']]['align']=='center')
                        $x = $x + ($parentWidth-$w)*0.5;
                    else if ($this->_tables[$param['num']]['align']=='right')
                        $x = $x + $parentWidth-$w;

                    $this->_tables[$param['num']]['curr_x'] = $x;
                }

                // calculate the height of the table
                $this->_tables[$param['num']]['height'] = array();

                // minimum of the height because of margins, and of the thead and tfoot height
                $h0 = $this->_tables[$param['num']]['marge']['t'] + $this->_tables[$param['num']]['marge']['b'];
                $h0+= $this->_tables[$param['num']]['thead']['height'] + $this->_tables[$param['num']]['tfoot']['height'];

                // max height of the page
                $max = $this->pdf->getH() - $this->pdf->getbMargin();

                // current position on the page
                $y = $this->_tables[$param['num']]['curr_y'];
                $height = $h0;

                // we get the height of each line
                for ($k=0; $k<count($this->_tables[$param['num']]['cases']); $k++) {

                    // if it is a TR of the thead or of the tfoot => skip
                    if (in_array($k, $this->_tables[$param['num']]['thead']['tr'])) continue;
                    if (in_array($k, $this->_tables[$param['num']]['tfoot']['tr'])) continue;

                    // height of the line
                    $th = 0;
                    $h = 0;
                    for ($i=0; $i<count($this->_tables[$param['num']]['cases'][$k]); $i++) {
                        $h = max($h, $this->_tables[$param['num']]['cases'][$k][$i]['h']);

                        if ($this->_tables[$param['num']]['cases'][$k][$i]['rowspan']==1)
                            $th = max($th, $this->_tables[$param['num']]['cases'][$k][$i]['h']);
                    }

                    // if the row does not fit on the page => new page
                    if ($y+$h+$height>$max) {
                        if ($height==$h0) $height = null;
                        $this->_tables[$param['num']]['height'][] = $height;
                        $height = $h0;
                        $y = $this->_margeTop;
                    }
                    $height+= $th;
                ...
                if ($height!=$h0 || $k==0) $this->_tables[$param['num']]['height'][] = $height;
            } else {
                // if we have tfoor, draw it
                if (count($this->_tables[$param['num']]['tfoot']['code'])) {
                    $tmpTR = $this->_tables[$param['num']]['tr_curr'];
                    $tmpTD = $this->_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    $this->_tables[$param['num']]['tr_curr'] = $this->_tables[$param['num']]['tfoot']['tr'][0];
                    $this->_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = $this->_tables[$param['num']]['tfoot']['code'];
                    ...
                    $this->_parsePos =     $oldParsePos;
                    $this->parsingHtml->code = $oldParseCode;
                    $this->_tables[$param['num']]['tr_curr'] = $tmpTR;
                    $this->_tables[$param['num']]['td_curr'] = $tmpTD;
                }

                // get the positions of the end of the table
                $x = $this->_tables[$param['num']]['curr_x'] + $this->_tables[$param['num']]['width'];
                if (count($this->_tables[$param['num']]['height'])>1)
                    $y = $this->_margeTop+$this->_tables[$param['num']]['height'][count($this->_tables[$param['num']]['height'])-1];
                else if (count($this->_tables[$param['num']]['height'])==1)
                    $y = $this->_tables[$param['num']]['curr_y']+$this->_tables[$param['num']]['height'][count($this->_tables[$param['num']]['height'])-1];
                else
                    $y = $this->_tables[$param['num']]['curr_y'];
        ...
        protected function _tag_open_COL($param)
        {
            ...
            for ($k=0; $k<$span; $k++)
                $this->_tables[$param['num']]['cols'][] = $param;
        }
        ...
        protected function _tag_open_TR($param, $other = 'tr')
        {
            ...
            $this->_tables[$param['num']]['tr_curr']++;
            $this->_tables[$param['num']]['td_curr']= 0;
            ...
            if (!$this->_subPart) {
                ...
                // Y after the row
                $ty=null;
                for ($ii=0; $ii<count($this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1]); $ii++) {
                    $ty = max($ty, $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$ii]['h']);
                }

                // height of the tfoot
                $hfoot = $this->_tables[$param['num']]['tfoot']['height'];

                // if the line does not fit on the page => new page
                if (!$this->_isInTfoot && $this->_tables[$param['num']]['td_y'] + $this->_tables[$param['num']]['marge']['b'] + $ty +$hfoot> $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    // new page
                    $this->_tables[$param['num']]['new_page'] = true;
                    $this->_setNewPage();

                    // new position
                    $this->_tables[$param['num']]['page']++;
                    $this->_tables[$param['num']]['curr_y'] = $this->pdf->getY();
                    $this->_tables[$param['num']]['td_y'] = $this->_tables[$param['num']]['curr_y']+$this->_tables[$param['num']]['marge']['t'];

                    // if we have the height of the tbale on the page => draw borders and background
                    if (isset($this->_tables[$param['num']]['height'][$this->_tables[$param['num']]['page']])) {
                        $old = $this->parsingCss->value;
                        $this->parsingCss->value = $this->_tables[$param['num']]['style_value'];

                        $this->_drawRectangle(
                            $this->_tables[$param['num']]['curr_x'],
                            $this->_tables[$param['num']]['curr_y'],
                            $this->_tables[$param['num']]['width'],
                            $this->_tables[$param['num']]['height'][$this->_tables[$param['num']]['page']],
                            $this->parsingCss->value['border'],
                            $this->parsingCss->value['padding'],
                            $this->_tables[$param['num']]['cellspacing']*0.5,
                            $this->parsingCss->value['background']
                        );
                ...
                if ($this->_tables[$param['num']]['new_page'] && count($this->_tables[$param['num']]['thead']['code'])) {
                    $this->_tables[$param['num']]['new_page'] = false;
                    $tmpTR = $this->_tables[$param['num']]['tr_curr'];
                    $tmpTD = $this->_tables[$param['num']]['td_curr'];
                    $oldParsePos = $this->_parsePos;
                    $oldParseCode = $this->parsingHtml->code;

                    $this->_tables[$param['num']]['tr_curr'] = $this->_tables[$param['num']]['thead']['tr'][0];
                    $this->_tables[$param['num']]['td_curr'] = 0;
                    $this->_parsePos = 0;
                    $this->parsingHtml->code = $this->_tables[$param['num']]['thead']['code'];
            ...
            // else (in a sub HTML)
                ...
                // prepare it
                $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1] = array();
                if (!isset($this->_tables[$param['num']]['corr'][$this->_tables[$param['num']]['corr_y']]))
                    $this->_tables[$param['num']]['corr'][$this->_tables[$param['num']]['corr_y']] = array();

                $this->_tables[$param['num']]['corr_x']=0;
                while(isset($this->_tables[$param['num']]['corr'][$this->_tables[$param['num']]['corr_y']][$this->_tables[$param['num']]['corr_x']]))
                    $this->_tables[$param['num']]['corr_x']++;
            }
        ...
        protected function _tag_close_TR($param)
        {
            ...
            if (!$this->_subPart) {
                ...
                $ty=null;
                for ($ii=0; $ii<count($this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1]); $ii++) {
                    if ($this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$ii]['rowspan']==1) {
                        $ty = $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$ii]['h'];
                    }
                }

                // new position
                $this->_tables[$param['num']]['td_x'] = $this->_tables[$param['num']]['curr_x']+$this->_tables[$param['num']]['marge']['l'];
                $this->_tables[$param['num']]['td_y']+= $ty;
                $this->_tables[$param['num']]['new_page'] = false;
            } else {
                $this->_tables[$param['num']]['corr_y']++;
            }
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            $param['cellpadding'] = $this->_tables[$param['num']]['cellpadding'].'mm';
            $param['cellspacing'] = $this->_tables[$param['num']]['cellspacing'].'mm';
            ...
            $x = $this->_tables[$param['num']]['td_curr'];
            $y = $this->_tables[$param['num']]['tr_curr']-1;
            ...
            if (in_array($other, array('td', 'th'))) {
                // id of the column
                $numCol = isset($this->_tables[$param['num']]['cases'][$y][$x]['Xr']) ? $this->_tables[$param['num']]['cases'][$y][$x]['Xr'] : $this->_tables[$param['num']]['corr_x'];

                // we get the properties of the COL tag, if exist
                if (isset($this->_tables[$param['num']]['cols'][$numCol])) {

                    $colParam = $this->_tables[$param['num']]['cols'][$numCol];

                    // for colspans => we get all the neede widths
                    $colParam['style']['width'] = array();
                    for ($k=0; $k<$colspan; $k++) {
                        if (isset($this->_tables[$param['num']]['cols'][$numCol+$k]['style']['width'])) {
                            $colParam['style']['width'][] = $this->_tables[$param['num']]['cols'][$numCol+$k]['style']['width'];
                        }
                    }
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if ($this->_tables[$param['num']]['border']) {
                    ...
                    $legacy['border']['b'] = $this->_tables[$param['num']]['border'];
                }
            }
            ...
            if ($collapse) {
                if (!$this->_subPart) {
                    if (
                        ($this->_tables[$param['num']]['tr_curr']>1 && !$this->_tables[$param['num']]['new_page']) ||
                        (!$this->_isInThead && count($this->_tables[$param['num']]['thead']['code']))
                    ) {
                        $this->parsingCss->value['border']['t'] = $this->parsingCss->readBorder('none');
                    }
                }

                if ($this->_tables[$param['num']]['td_curr']>0) {
            ...
            $marge = array();
            $marge['t'] = $this->parsingCss->value['padding']['t']+0.5*$this->_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['t']['width'];
            $marge['r'] = $this->parsingCss->value['padding']['r']+0.5*$this->_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['r']['width'];
            $marge['b'] = $this->parsingCss->value['padding']['b']+0.5*$this->_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['b']['width'];
            $marge['l'] = $this->parsingCss->value['padding']['l']+0.5*$this->_tables[$param['num']]['cellspacing']+$this->parsingCss->value['border']['l']['width'];

            // if we are in a sub HTML
            if ($this->_subPart) {
                // new position in the table
                $this->_tables[$param['num']]['td_curr']++;
                $this->_tables[$param['num']]['cases'][$y][$x] = array();
                $this->_tables[$param['num']]['cases'][$y][$x]['w'] = 0;
                $this->_tables[$param['num']]['cases'][$y][$x]['h'] = 0;
                $this->_tables[$param['num']]['cases'][$y][$x]['dw'] = 0;
                $this->_tables[$param['num']]['cases'][$y][$x]['colspan'] = $colspan;
                $this->_tables[$param['num']]['cases'][$y][$x]['rowspan'] = $rowspan;
                $this->_tables[$param['num']]['cases'][$y][$x]['Xr'] = $this->_tables[$param['num']]['corr_x'];
                $this->_tables[$param['num']]['cases'][$y][$x]['Yr'] = $this->_tables[$param['num']]['corr_y'];

                // prepare the mapping for rowspan and colspan
                for ($j=0; $j<$rowspan; $j++) {
                    for ($i=0; $i<$colspan; $i++) {
                        $this->_tables[$param['num']]['corr']
                            [$this->_tables[$param['num']]['corr_y']+$j]
                            [$this->_tables[$param['num']]['corr_x']+$i] = ($i+$j>0) ? '' : array($x,$y,$colspan,$rowspan);
                    }
                }
                $this->_tables[$param['num']]['corr_x']+= $colspan;
                while (isset($this->_tables[$param['num']]['corr'][$this->_tables[$param['num']]['corr_y']][$this->_tables[$param['num']]['corr_x']])) {
                    $this->_tables[$param['num']]['corr_x']++;
                }
                ...
                // new position in the table
                $this->_tables[$param['num']]['td_curr']++;
                $this->_tables[$param['num']]['td_x']+= $this->_tables[$param['num']]['cases'][$y][$x]['dw'];

                // borders and background of the TD
                $this->_drawRectangle(
                    $this->_tables[$param['num']]['td_x'],
                    $this->_tables[$param['num']]['td_y'],
                    $this->_tables[$param['num']]['cases'][$y][$x]['w'],
                    $this->_tables[$param['num']]['cases'][$y][$x]['h'],
                    $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'],
                    $this->_tables[$param['num']]['cellspacing']*0.5,
                    $this->parsingCss->value['background']
                ...
                $this->parsingCss->value['width'] = $this->_tables[$param['num']]['cases'][$y][$x]['w'] - $marge['l'] - $marge['r'];

                // marges = size of the TD
                $mL = $this->_tables[$param['num']]['td_x']+$marge['l'];
                $mR = $this->pdf->getW() - $mL - $this->parsingCss->value['width'];
                $this->_saveMargin($mL, 0, $mR);

                // position of the content, from vertical-align
                $hCorr = $this->_tables[$param['num']]['cases'][$y][$x]['h'];
                $hReel = $this->_tables[$param['num']]['cases'][$y][$x]['real_h'];
                switch($this->parsingCss->value['vertical-align'])
                {
                    case 'bottom':
                        $yCorr = $hCorr-$hReel;
                ...
                $x = $this->_tables[$param['num']]['td_x']+$marge['l'];
                $y = $this->_tables[$param['num']]['td_y']+$marge['t']+$yCorr;
                $this->pdf->setXY($x, $y);
                $this->_setNewPositionForNewLine();
            }
        ...
        protected function _tag_close_TD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$this->_tables[$param['num']]['td_curr']-1]['w'] = max(array($w0, $w2));
                $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$this->_tables[$param['num']]['td_curr']-1]['h'] = max(array($h0, $h2));

                // real position of the content
                $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$this->_tables[$param['num']]['td_curr']-1]['real_w'] = $w0;
                $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$this->_tables[$param['num']]['td_curr']-1]['real_h'] = $h0;
                ...
                $this->_tables[$param['num']]['td_x']+= $this->_tables[$param['num']]['cases'][$this->_tables[$param['num']]['tr_curr']-1][$this->_tables[$param['num']]['td_curr']-1]['w'];
            }
